    ...
    public static void SetupErrorResponses(WireMockServer server)
    {
        ...
        server
            ...
            .RespondWith(Response.Create()
                .WithStatusCode(500)
                .WithHeader("Content-Type", "application/json")
                .WithBodyAsJson(new { error = "Internal Server Error", message = "An internal server error occurred." }));
    ...
    private static object GenerateLargeJsonResponse()
    {
        ...
        for (int i = 1; i <= 100; i++)
        {
            items.Add(new
            {
                id = i,
                name = $"Item {i}",
                description = $"This is a test item number {i} with some additional content to make the response larger.",
